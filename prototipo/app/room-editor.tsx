...
export default function RoomEditor(){
  ...
  return <main className="editor">
    ...
    <div className="workspace"><aside className="properties"><h2>Ambientes · {house.rooms.length} de 8</h2>
      ...
      <p className="hint">Medidas em metros. Porta externa e janela: distância da face interna esquerda. Enter aplica. Cada divisão tem uma porta central de 0,90 m.</p><p className="hint">Nesta etapa, cômodos alinhados lado a lado. Remover um ambiente aproxima os restantes.</p></section>
    ...
    <div className={`views ${view==='split'?'split':''}`}>
      {view!=='3d'&&<section className="view plan"><div className="view-title">Planta · {current.name}</div><svg className="plan-svg" viewBox={`${-layout.width/2-t-1} ${-d/2-t-1} ${layout.width+2*t+2} ${d+2*t+2}`} aria-label="Planta dos ambientes conectados">
        ...
        {meshes.map((m,i)=>{const zs=m.vertices.map(v=>v[2]);if(Math.min(...zs)>1.2||Math.max(...zs)<1.2)return null;const xs=m.vertices.map(v=>v[0]),ys=m.vertices.map(v=>-v[1]);const opening=m.id.includes(':D01')||m.id.includes(':J01')||m.id.startsWith('link:');return <rect key={i} x={Math.min(...xs)} y={Math.min(...ys)} width={Math.max(...xs)-Math.min(...xs)} height={Math.max(...ys)-Math.min(...ys)} fill={selected===m.id?'#168ac0':opening?'#a38b6b':'#657a88'} onClick={()=>pick(m.id)}/>;})}
      ...
      {view!=='plan'&&<section className="view three"><Viewport walls={[]} meshes={meshes} selected={selected} onSelect={pick} reset={reset} hideFloor/><div className="view-caption">Arraste para orbitar · roda para aproximar</div></section>}
